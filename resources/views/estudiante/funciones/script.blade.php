<script type="text/javascript">
    $(document).ready(function () {
        $('#modal_crear_estudiante').on('shown.bs.modal', function () {
            $('#documento').trigger('focus');
        });
        function getEstudiantes() {
            var tabla_estudiantes = $('#tabla-estudiantes'), dt_estudiantes;
            if (tabla_estudiantes.length) {
                dt_estudiantes = tabla_estudiantes.DataTable({
                    ajax: {
                        url: '{{ route("estudiantes.show") }}',
                        type: 'GET',
                        dataType: 'json',
                        ...
                        },
                        {
                            ...
                            render: function (data, type, full, meta) {
                                ...
                                var botones =   `<div class="btn-group" role="group" aria-label="Basic example">
                                                    <button type="button" class="btn btn-sm btn-label-warning EstudianteEditar" id="EstudianteEditar" title="Editar Estudiante" data-bs-toggle="modal" data-bs-target="#modal_editar_estudiante">
                                                        <i class="bx bxs-edit bx-sm"></i>
                                                    </button>
                                                    <button type="button" class="btn btn-sm btn-label-danger EstudianteEliminar" id="EstudianteEliminar" title="Eliminar Estudiante">
                    ...
                    buttons: [
                        {
                            text: '<i class="bx bx-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Nuevo Estudiante</span>',
                            ...
                            buttons: [
                                {
                                    ...
                                    customize: function (win) {
                                        ...
                                        $(win.document.body).prepend('<div class="table-title"><h2 style="text-align: center;" hidden></h2><h3 style="text-align: center;">TABLA ESTUDIANTES</h3></div><br>');
                                        $(win.document.body)
        ...
        $(document).on('click', '#GuardarEstudiante', function () {
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        if (data.resultado === 'si') {
                            Swal.fire({
                                title: 'Advertencia!',
                                text: 'El estudiante ' + nombres + ' ' + apellidos + 'con ' + ci + ' ya se encuentra registrado.',
                                icon: 'warning',
                                showConfirmButton: false,
                                allowOutsideClick: false,
                                allowEscapeKey: false,
                            ...
                            let tabla = $('.tabla-estudiantes').DataTable();
                            tabla.destroy();
                            $('#modal_crear_estudiante .btn-close').trigger('click');
                            $("#formulario_crear_estudiante").trigger('reset');
                            getEstudiantes();
                        }
                    },
        ...
        $(document).on('click', '.btnCancelarEdit', function () {
            $('#formulario_editar_estudiante').trigger('reset');
        });

        function getEstudiante(id) {
            mostrarCargando();
            $.ajax({
                url: '{{ route("estudiante.edit", ":id") }}'.replace(":id", id),
                type: 'GET',
                dataType: 'json',
                success: (data) => {
                    $('.idUpdate').val(data.id);
                    $('.generoUpdate').val(data.genero);
        ...
        $(document).on('click', '.EstudianteEditar', function () {
            let id;
        ...
        $(document).on('click', '.ActualizarEstudiante', function () {
            let id = $('.idUpdate').val();
            let ci = $('.documentoUpdate').val();
            let genero = $('.generoUpdate').val();
            ...
            if (vacio != "") {
                ...
                $.ajax({
                    url: '{{ route("estudiante.update", ":id") }}'.replace(":id", id),
                    type: 'POST',
                    data: datos,
                    processData: false,
                    contentType: false,
                    ...
                    success: (data) => {
                        ...
                        let tabla = $('.tabla-estudiantes').DataTable();
                        tabla.destroy();
                        $('#modal_editar_estudiante .btn-close').trigger('click');
                        $("#formulario_editar_estudiante").trigger('reset');
                        getEstudiantes();
                    },
                    error: function (xhr, textStatus, errorThrown) {
        ...
        function cambiarEstado(id) {
            $.ajax({
                url: '{{ route("estudiante.estado", ":id") }}'.replace(":id", id),
                type: "GET",
                dataType: 'json',
                success: (data) => {
        ...
        $(document).on('click', '.desactivar', function () {
            ...
            Swal.fire({
                ...
                text: '¿Esta seguro de deshabilitar al estudiante NRO: ' + documento + '?',
                icon: 'question',
        ...
        $(document).on('click', '.activar', function () {
            ...
            Swal.fire({
                ...
                text: '¿Esta seguro de habilitar al estudiante NRO: ' + documento + '?',
                icon: 'question',
                showCancelButton: true,
        ...
        function eliminarEstudiante(id) {
            $.ajax({
                url: '{{ route("estudiante.destroy", ":id") }}'.replace(":id", id),
                type: "GET",
                dataType: 'json',
                success: (data) => {
                    if (data.resultado == 'ok') {
        ...
        $(document).on('click', '.EstudianteEliminar', function () {
            let id;
            if ($(this).closest('.modal').length > 0) {
                id = $('.modal-body table tbody tr:nth-child(1) td:nth-child(2)').text().trim();
                documento = $('.modal-body table tbody tr:nth-child(2) td:nth-child(2)').text().trim();
            ...
            Swal.fire({
                ...
                text: '¿Esta seguro de borrar los datos del estudiante NRO: ' + documento + '?',
                icon: 'warning',
                showCancelButton: true,
